        ...
        <div className="space-y-2">
          {(data?.low_stock_items || []).slice(0, 8).map((item) => (
            <div key={item.id} className="flex items-center justify-between py-2 border-b border-border/50 last:border-0">
              ...
              <span className="text-sm font-bold text-accent-red">{SW.dashibodi.zilizobaki(item.available_qty)}</span>
            </div>
          ))}
          {(!data?.low_stock_items || data.low_stock_items.length === 0) && !loading && (
            <p className="text-sm text-text-muted text-center py-4">{SW.dashibodi.bidhaaZikoSawa}</p>
          )}
        </div>
      </Card>
    </PageWrapper>
