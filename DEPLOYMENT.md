...
Login/forgot-password rate limiting uses in-process memory only — not
shared across gunicorn workers, and resets on restart. That's an accepted
tradeoff for now; there's no Redis dependency to configure.
